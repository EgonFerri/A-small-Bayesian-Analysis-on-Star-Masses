...
No one of these answers the questions, so we cann apply **d-separation** procedure to verify it.
...
Now we can perform the sampling following the aforementioned forwarding rule.
...
M = 200000
sample_ <- sapply(1:M, ancestral_sampling)
```

Now that we got our sample of the joint distribution we can check what's happening at the marginal distributions of $\mathbf{X}$:
...
curve(dnorm(x  ,mean = mean(sample_[5,]), sd = sd(sample_[5,])), col='darkred',type='l', lwd=2, add = T)
```

These distributions seems to be pretty normal. We can check it by making a [Kolmogorov-Smirnov test](https://en.wikipedia.org/wiki/Kolmogorov%E2%80%93Smirnov_test) that we remeber  examines if scores
are likely to follow some distribution in some population. There are various kind of this test, we're interested in the **one sample Kolmogorov-Smirnov** test for testing if a variable follows a given distribution in a population. This "given distribution" is usually -and in our case- the normal distribution, hence "Kolmogorov-Smirnov normality test".

In a KS test, the null hypothesis is that our sample follows the distribution we compare it against. So higher is the statistic, higher is valid the rejection of the null. In fact, without going too much deep, we can consider the statistics as a measure of the distance (in particular the $\operatorname{sup}$) between the empirical distributions of the sample and the distribution we think it follows.In particular, we decide we need a **Type I ** error probability $\le 0.05$ and so our rejection rule is reject high statistic values $H_0$ if 

$$ p-value \le 0.05$$

Let's run the test comparing the empirical distibution of $X_1$ against a $N(\bar{X}_1, S_n^2(X_1))$

```{r}
ks.test(x = sample_[4,], y = "pnorm", mean = mean(sample_[4,]), sd = sd(sample_[4,]))
```

As we can see, we're pretty sure that the component of $\mathbf{X}$ are normally distribuited.
...
**Da togliere se sei d'accordo (controlla l'altro sia giusto)**
```{r}
z <- (sample_[4,]-mean(sample_[4,]))/sd(sample_[4,])
qqnorm(z, col='orange'); qqline(z, col = 'darkred', lwd=1)
```
```{r}
ggplot() +
...
The sample seems normal, but we have more xtreme values in respect to a "true perfect normal" we can try to fit a t-student model to see if we can reconduct this heavy tail to that.
...
Nothing to do: the fit-distr gives us a t-student with 22 degrees of freedom(aproximable to a normal).
We have to accept this more-than-normal extreme values and live with it.
...
The fitting is pretty good and the model seems to be the right one.
...
The first step is loading the data, setting the constants and the initial points of the chains, in this case 3.
...
$$ \operatorname{E}[\boldsymbol{\mu},\sigma^2|\mathbf{Y}_n]= \arg\min_{d} [\operatorname{MSE(\boldsymbol{\mu},\sigma^2,d(\mathbf{Y}_n))]} $$

- The median of the posterior is the best estimator in the Absolute Loss ($L_1$) sense due to the fact that:

$$ \operatorname{Median}[\boldsymbol{\mu},\sigma^2|\mathbf{Y}_n]= \arg\min_{d} [\operatorname{L_1(\boldsymbol{\mu},\sigma^2,d(\mathbf{Y}_n))]} $$
The obtained estimates of this quantities are:

<img src="https://i.imgur.com/Val3rEe.jpg"\>
...
sigma_jags<-fit$BUGSoutput$mean[[2]]
```
...
- The $LogN(\hat\mu_{JUGS},\hat\sigma^2_{JUGS})$ (mean values).

```{r echo=FALSE}
hist(star.data, prob = T, col = "orange", border = "white", breaks = 60, xlab = "Star Masses", main = "Star Masses Distribution")
curve(dlnorm(x  ,meanlog = mu_hat,sdlog = sqrt(sigma2_hat)), col='darkred',type='l', lwd=3, add = T)
curve(dlnorm(x  ,meanlog = mu_jags,sdlog = sigma_jags), col='orchid',type='l', lwd=3, add = T)
curve(dlnorm(x  ,meanlog = -1.255,sdlog = .9452), col='lightsalmon',type='l', lwd=3, add = T)

legend("topright", legend = c("Empirical Distribution", "MLE","JAGS","BUGS"), col = c("orange", "darkred", "orchid", "lightsalmon"),lty = 1, cex = 0.8)
```

The distributions are very close (obviously JUGS and BUGS are equals) and this is what we expect. In both cases we are letting data speak for themselves, in fact this is an intrinsic property of the ML technique and a derived property for Bayesian Inference thanks to the choosen non-informative priors.
...
The idea is letting vary $\mu_0$,$\sigma_0$, $\alpha$ and $\beta$ and checking how the estimate $\bar\mu$ and $\bar\sigma^2$ react to changes. 
...
$$ \bar{\bar\mu}(\theta|\Theta - \theta) = \frac{1}{N}\sum_{i=1}^{N}\bar\mu(\theta|\Theta - \theta),\,\,\,\,\,\,\,\bar{\bar\sigma}(\theta|\Theta - \theta) = \frac{1}{N}\sum_{i=1}^{N}\bar\sigma(\theta|\Theta - \theta)$$
...
ab_var <- matrix(c(runif(2,0.001,10),runif(2,0.001,10)),nrow=2)
...
So how does this help us to generate realizations of **our model** from the joint posterior distribution in a Bayesian analysis?
...
## How Markov Chain lets you find your favourite video of the Royal Wedding: Page Rank
...
$$p_{i,j} = \mathbb{P}(\theta(t+1) = j|\theta(t)=i)$$
...
At this point, the stationary distribution related to this transition matrix represents the score of the page.
